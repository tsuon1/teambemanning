    ...
    <section className="bg-secondary section-padding">
      <div className="container-wide">
            ...
            {visibleCards.map((card) => (
              <Link key={card.title} to={card.href} className="group bg-background border border-border rounded-2xl overflow-hidden hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-all duration-200">
                <div className="aspect-[16/10] overflow-hidden relative">
                  ...
                  <span className="absolute top-3 left-3 bg-foreground text-background text-xs font-bold px-3 py-1 rounded-none">{card.badge}</span>
                </div>
                <div className="p-5">
          ...
          {showNav && visibleCount >= 5 && (
            <div className="flex justify-center gap-2 mt-6">
              {Array.from({ length: maxStart + 1 }, (_, i) => (
                <button key={i} onClick={() => setStartIndex(i)} className={`w-2 h-2 rounded-none transition-colors ${i === startIndex ? "bg-foreground" : "bg-foreground/20"}`} aria-label={`Go to ${i + 1}`} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="text-center mt-10">
          <Link to="/nyheter" className="inline-flex items-center bg-brand text-white font-medium text-sm px-7 py-2.5 rounded-none hover:scale-105 transition-transform">
