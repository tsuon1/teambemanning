      ...
      <div className="container-wide">
        <div className="grid md:grid-cols-2 gap-8 items-center pb-14 lg:pb-20">
          ...
          <form className="flex gap-3 items-center" onSubmit={(e) => { e.preventDefault(); setEmail(""); }}>
            <input type="email" placeholder={t("footer.emailPlaceholder")} value={email} onChange={(e) => setEmail(e.target.value)} className="flex-1 px-4 py-3 bg-background/10 text-background rounded-[2px] border border-background/20 outline-none text-sm placeholder:text-background/40" required />
            <button type="submit" className="px-5 py-3 bg-background text-foreground font-medium text-sm rounded-[2px] hover:bg-background/90 transition-colors flex items-center gap-1.5">
